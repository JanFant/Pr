...
#define internal1_m315_tx (*((psfloat)(InternalBuf+0x9))) /*tx - время накопленное сек*/
#define internal1_m315_y0 (*((pschar)(InternalBuf+0xE))) /*y0*/
#define internal1_m212_tx (*((psfloat)(InternalBuf+0x10))) /*tx - время накопленное сек*/
#define internal1_m212_y0 (*((pschar)(InternalBuf+0x15))) /*y0*/
#define internal1_m210_tx (*((psfloat)(InternalBuf+0x17))) /*tx - время накопленное сек*/
#define internal1_m210_y0 (*((pschar)(InternalBuf+0x1C))) /*y0*/
#define internal1_m252_tx (*((psfloat)(InternalBuf+0x1E))) /*tx - время накопленное сек*/
...
#define internal1_m120_y1 (*((psbool)(InternalBuf+0x59))) /*y1 - внутренний параметр*/
...
#define internal1_m162_y1 (*((psbool)(InternalBuf+0x61))) /*y1 - внутренний параметр*/
int InternalBufSize = 99;

/* Определение сигналов */
#define signal_A0IT02IZ2     (*((psint)(PBSTART+0x0)))  /*                       Температура АЗ1-2 */
#define signal_A0CT01IZ2     (*((psfloat)(PBSTART+0x3)))  /*                       Температура АЗ1-1 */
#define signal_A0VT61LZ2     (*((psbool)(PBSTART+0x8)))  /*                       ПС по температуре в АЗ1 */
#define signal_A0VT71LZ2     (*((psbool)(PBSTART+0xa)))  /*                       АС по температуре в АЗ1 */
#define signal_A1VP41LZ2     (*((psbool)(PBSTART+0xc)))  /*                       Давление СБРОС ББ1 ниже АС */
#define signal_B1VP41LZ2     (*((psbool)(PBSTART+0xe)))  /*                       Давление СБРОС ББ2 ниже АС */
...
#define signal_A2VP41LZ2     (*((psbool)(PBSTART+0x14)))  /*                       Давление СБРОС РБ1 ниже АС */
#define signal_A2CP01RZ2     (*((psfloat)(PBSTART+0x16)))  /*                       Текущее давление СБРОС РБ1 */
#define signal_A2VP51LZ2     (*((psbool)(PBSTART+0x1b)))  /*                       Давление СБРОС РБ1 ниже ПС */
#define signal_B2VP41LZ2     (*((psbool)(PBSTART+0x1d)))  /*                       Давление СБРОС РБ2 ниже АС */
#define signal_B2CP01RZ2     (*((psfloat)(PBSTART+0x1f)))  /*                       Текущее давление СБРОС РБ2 */
#define signal_B2VP51LZ2     (*((psbool)(PBSTART+0x24)))  /*                       Давление СБРОС РБ2 ниже ПС */
#define signal_B2VP81LZ2     (*((psbool)(PBSTART+0x26)))  /*                       Давление СБРОС РБ2 в норме */
#define signal_A2VP81LZ2     (*((psbool)(PBSTART+0x28)))  /*                       Давление СБРОС РБ1 в норме */
#define signal_A2IP01IZ2     (*((psint)(PBSTART+0x2a)))  /*                       Текущее давление СБРОС РБ1 */
#define signal_B2IP01IZ2     (*((psint)(PBSTART+0x2d)))  /*                       Текущее давление СБРОС РБ2 */
#define signal_R0ET01LZ2     (*((psbool)(PBSTART+0x30)))  /*                       Признак наличия неисправности по температуре  АЗ1 */
#define signal_B0IT02IZ2     (*((psint)(PBSTART+0x32)))  /*                       Температура АЗ2-2 */
#define signal_A0VP41LZ2     (*((psbool)(PBSTART+0x35)))  /*                       Давление АЗ1 ниже АС */
#define signal_B0VP41LZ2     (*((psbool)(PBSTART+0x37)))  /*                       Давление АЗ2 ниже АС */
...
#define signal_R0ET02LZ2     (*((psbool)(PBSTART+0x3d)))  /*                       Признак наличия неисправности по температуре АЗ2 */
#define signal_B0EP02IZ2     (*((psint)(PBSTART+0x3f)))  /*                       Индикатор -Давление  АЗ2 ниже АС */
#define signal_A0EP02IZ2     (*((psint)(PBSTART+0x42)))  /*                       Индикатор -Давление  АЗ1 ниже АС */
#define signal_A0VP71LZ2     (*((psbool)(PBSTART+0x45)))  /*                       АЗ по АС любого давления для АЗ1 */
#define signal_B0VP71LZ2     (*((psbool)(PBSTART+0x47)))  /*                       АЗ по АС любого давления для АЗ2 */
...
#define signal_R0VN01RZ2     (*((psfloat)(PBSTART+0x81)))  /*                       Уровень мощности по каналу 1 */
#define signal_R0VN02RZ2     (*((psfloat)(PBSTART+0x86)))  /*                       Уровень мощности по каналу 2 */
#define signal_R0VN03RZ2     (*((psfloat)(PBSTART+0x8b)))  /*                       Уровень мощности по каналу 3 */
#define signal_R0VN04RZ2     (*((psfloat)(PBSTART+0x90)))  /*                       Уровень мощности по каналу 4 */
#define signal_R0VN05RZ2     (*((psfloat)(PBSTART+0x95)))  /*                       Период разгона по каналу 1 */
#define signal_R0VN06RZ2     (*((psfloat)(PBSTART+0x9a)))  /*                       Период разгона по каналу 2 */
#define signal_R0VN07RZ2     (*((psfloat)(PBSTART+0x9f)))  /*                       Период разгона по каналу 3 */
#define signal_R0VN08RZ2     (*((psfloat)(PBSTART+0xa4)))  /*                       Период разгона по каналу 4 */
#define signal_R0VN61LZ2     (*((psbool)(PBSTART+0xa9)))  /*                       ПС по мощности канал 1 */
#define signal_R0VN62LZ2     (*((psbool)(PBSTART+0xab)))  /*                       ПС по мощности канал 2 */
#define signal_R0VN63LZ2     (*((psbool)(PBSTART+0xad)))  /*                       ПС по мощности канал 3 */
#define signal_R0VN64LZ2     (*((psbool)(PBSTART+0xaf)))  /*                       ПС по мощности канал 4 */
#define signal_R0VN65LZ2     (*((psbool)(PBSTART+0xb1)))  /*                       ПС по периоду разгона канал 1 */
#define signal_R0VN66LZ2     (*((psbool)(PBSTART+0xb3)))  /*                       ПС по периоду разгона канал 2 */
#define signal_R0VN67LZ2     (*((psbool)(PBSTART+0xb5)))  /*                       ПС по периоду разгона канал 3 */
#define signal_R0VN68LZ2     (*((psbool)(PBSTART+0xb7)))  /*                       ПС по периоду разгона канал 4 */
#define signal_R0VN09RZ2     (*((psfloat)(PBSTART+0xb9)))  /*                       Усредненный период разгона */
...
#define signal_R0AD03LZ2     (*((psbool)(PBSTART+0xc2)))  /*                       Имитация прихода на ВУ ИС */
#define signal_B3IS11LDU     (*((psbool)(PBSTART+0xc4)))  /*                       Приход на ВУ ИС2 */
#define signal_A3VZ31LZ2     (*((psbool)(PBSTART+0xc6)))  /*                       АЗ от ВУ ИС1 */
#define signal_B3VZ31LZ2     (*((psbool)(PBSTART+0xc8)))  /*                       АЗ от ВУ ИС2 */
#define signal_R0AD05LZ2     (*((psbool)(PBSTART+0xca)))  /*                       Имитация ухода с НУП ИС */
#define signal_A3VZ11LZ2     (*((psbool)(PBSTART+0xcc)))  /*                       АЗ от НУП ИС1 */
#define signal_B3VZ11LZ2     (*((psbool)(PBSTART+0xce)))  /*                       АЗ от НУП ИС2 */
#define signal_B3IS22LDU     (*((psbool)(PBSTART+0xd0)))  /*                       Приход на НУП ИС2 */
#define signal_R0VN69LZ2     (*((psbool)(PBSTART+0xd2)))  /*                       ПС по усредненному периоду */
#define signal_R0VN79LZ2     (*((psbool)(PBSTART+0xd4)))  /*                       АС по усредненному периоду */
#define signal_R0VR02RZ2     (*((psfloat)(PBSTART+0xd6)))  /*                       Усредненная реактивность */
#define signal_R0VZ71LZ2     (*((psbool)(PBSTART+0xdb)))  /*                       Обобщенный сигнал АЗ 2 канала */
#define signal_R0VN70LZ2     (*((psbool)(PBSTART+0xdd)))  /*                       АЗ по АС мощности РУ */
#define signal_R0VN80LZ2     (*((psbool)(PBSTART+0xdf)))  /*                       АЗ по АС периода разгона РУ */
#define signal_A1EE01LS1     (*((psbool)(PBSTART+0xe1)))  /*                       Исправность АКНП канал 1 */
#define signal_A1EE01LS4     (*((psbool)(PBSTART+0xe3)))  /*                       Исправность АКНП канал 4 */
#define signal_A1EE01LS3     (*((psbool)(PBSTART+0xe5)))  /*                       Исправность АКНП канал 3 */
#define signal_A0EE01LZ2     (*((psbool)(PBSTART+0xe7)))  /*                       Исправность АКНП1 */
#define signal_A0EE04LZ2     (*((psbool)(PBSTART+0xe9)))  /*                       Исправность АКНП4 */
#define signal_A0EE02LZ2     (*((psbool)(PBSTART+0xeb)))  /*                       Исправность АКНП2 */
#define signal_A0EE03LZ2     (*((psbool)(PBSTART+0xed)))  /*                       Исправность АКНП3 */
#define signal_A1EE01LS2     (*((psbool)(PBSTART+0xef)))  /*                       Исправность АКНП  канал 2 */
#define signal_A0EE05LZ2     (*((psbool)(PBSTART+0xf1)))  /*                       Исправность сети АКНП1 */
#define signal_A0EE06LZ2     (*((psbool)(PBSTART+0xf3)))  /*                       Исправность сети АКНП2 */
#define signal_A0EE07LZ2     (*((psbool)(PBSTART+0xf5)))  /*                       Исправность сети АКНП3 */
#define signal_A0EE08LZ2     (*((psbool)(PBSTART+0xf7)))  /*                       Исправность сети АКНП4 */
#define signal_A0VZ71LZ2     (*((psbool)(PBSTART+0xf9)))  /*                       Обобщенный сигнал АС по АЗ1 */
#define signal_A0EE00LZ2     (*((psbool)(PBSTART+0xfb)))  /*                       Исправность АКНП АЗ1 */
#define signal_B0EE00LZ2     (*((psbool)(PBSTART+0xfd)))  /*                       Исправность АКНП АЗ2 */
#define signal_B0VZ71LZ2     (*((psbool)(PBSTART+0xff)))  /*                       Обобщенный сигнал АС по АЗ2 */
#define signal_A0VN71LZ2     (*((psbool)(PBSTART+0x101)))  /*                       АЗ по АС мощности  АЗ1 */
#define signal_B0VN71LZ2     (*((psbool)(PBSTART+0x103)))  /*                       АЗ по АС мощности  АЗ2 */
#define signal_A0VN81LZ2     (*((psbool)(PBSTART+0x105)))  /*                       АЗ по АС периода разгона АЗ1 */
#define signal_B0VN81LZ2     (*((psbool)(PBSTART+0x107)))  /*                       АЗ по АС периода разгона АЗ2 */
#define signal_R0VZ05UZ2     (*((pslong)(PBSTART+0x109)))  /*                       Индикатор причины сброса */
#define signal_R0VN11RZ2     (*((psfloat)(PBSTART+0x10e)))  /*                       Средняя мощность по БАЗ1 */
#define signal_R0VP73LZ2     (*((psbool)(PBSTART+0x113)))  /*                       ПС давления для РУ */
#define signal_B0CT01IZ2     (*((psfloat)(PBSTART+0x115)))  /*                       Температура АЗ2-1 */
#define signal_B0VT61LZ2     (*((psbool)(PBSTART+0x11a)))  /*                       ПС по температуре в АЗ2 */
#define signal_B0VT71LZ2     (*((psbool)(PBSTART+0x11c)))  /*                       АС по температуре в АЗ2 */
#define signal_A1VN71LS1     (*((psbool)(PBSTART+0x11e)))  /*                       Блокировка автоматического подъёма ББ канал 1 */
#define signal_A1VN71LS2     (*((psbool)(PBSTART+0x120)))  /*                       Блокировка автоматического подъёма ББ канал 2 */
#define signal_A1VN71LS3     (*((psbool)(PBSTART+0x122)))  /*                       Блокировка автоматического подъёма ББ канал 3 */
#define signal_A1VN71LS4     (*((psbool)(PBSTART+0x124)))  /*                       Блокировка автоматического подъёма ББ канал 4 */
#define signal_A1VN71LZ2     (*((psbool)(PBSTART+0x126)))  /*                       Блокировка автоматического подъёма ББ */
#define signal_A3VZ15LZ2     (*((psbool)(PBSTART+0x128)))  /*                       Имитация АЗ от НУП ИС */
#define signal_A3VZ13LZ2     (*((psbool)(PBSTART+0x12a)))  /*                       Имитация АЗ от ВУ ИС */
#define signal_R0VW13LDU     (*((psbool)(PBSTART+0x12c)))  /*                       Индикация выбора АЗ1 или АЗ1+АЗ2 */
#define signal_R0VW23LDU     (*((psbool)(PBSTART+0x12e)))  /*                       Индикация выбора АЗ2 или АЗ1+АЗ2 */
#define signal_R7II73LZ2     (*((psbool)(PBSTART+0x130)))  /*                       Сработала АС II УР РАД */
#define signal_R7II71LZ2     (*((psbool)(PBSTART+0x132)))  /*                       Сработала АС IУР РАД (датчик 1) */
#define signal_R7II72LZ2     (*((psbool)(PBSTART+0x134)))  /*                       Сработала АС IУР РАД (датчик 2) */
#define signal_R0AD21LDU     (*((psbool)(PBSTART+0x136)))  /*                       Подключить защиту от II УР */
#define signal_R0MD34LP1     (*((psbool)(PBSTART+0x138)))  /*                       Кнопка Квитировать */
#define signal_R0AD14LDU     (*((psbool)(PBSTART+0x13a)))  /*                       Имитация срабатывания верхней АС II УР */
#define signal_R0AD16LDU     (*((psbool)(PBSTART+0x13c)))  /*                       Имитация срабатывания верхней АС I УР */
#define signal_R6IS66LZZ     (*((psbool)(PBSTART+0x13e)))  /*                       Исправность ВИП 4,0 (№5) ССДИ-1-1 */
#define signal_R6IS67LZZ     (*((psbool)(PBSTART+0x140)))  /*                       Исправность ВИП 4,0 (№6) ССДИ-1-2 */
#define signal_R6IS68LZZ     (*((psbool)(PBSTART+0x142)))  /*                       Исправность ВИП 4,0 (№7) ССДИ-2 */
#define signal_R0VN71LZ2     (*((psbool)(PBSTART+0x144)))  /*                       АС по мощности канал 1 */
#define signal_R0VN75LZ2     (*((psbool)(PBSTART+0x146)))  /*                       АС по периоду разгона канал 1 */
#define signal_R0VN72LZ2     (*((psbool)(PBSTART+0x148)))  /*                       АС по мощности канал 2 */
#define signal_R0VN76LZ2     (*((psbool)(PBSTART+0x14a)))  /*                       АС по периоду разгона канал  2 */
#define signal_R0VN73LZ2     (*((psbool)(PBSTART+0x14c)))  /*                       АС по мощности канал 3 */
#define signal_R0VN74LZ2     (*((psbool)(PBSTART+0x14e)))  /*                       АС по мощности канал 4 */
#define signal_R0VN77LZ2     (*((psbool)(PBSTART+0x150)))  /*                       АС по периоду разгона канал  3 */
#define signal_R0VN78LZ2     (*((psbool)(PBSTART+0x152)))  /*                       АС по периоду разгона канал  4 */
#define signal_R0AD04LZ2     (*((psbool)(PBSTART+0x154)))  /*                       Имитация срабатывания верхней АС II УР */
#define signal_R0EE01LZ2     (*((psbool)(PBSTART+0x156)))  /*                       Питание  АКНП1  отключить */
#define signal_R0EE02LZ2     (*((psbool)(PBSTART+0x158)))  /*                       Питание  АКНП2  отключить */
#define signal_R0EE03LZ2     (*((psbool)(PBSTART+0x15a)))  /*                       Питание  АКНП3  отключить */
#define signal_R0EE04LZ2     (*((psbool)(PBSTART+0x15c)))  /*                       Питание  АКНП4  отключить */
#define signal_R0AD14LZ2     (*((psbool)(PBSTART+0x15e)))  /*                       Имитация АЗ от IIУР */
#define signal_R0AD15LZ2     (*((psbool)(PBSTART+0x160)))  /*                       Имитация АЗ от 1УР (датчик1) */
#define signal_R0AD16LZ2     (*((psbool)(PBSTART+0x162)))  /*                       Имитация АЗ от 1УР (датчик2) */
#define signal_R0MW12IP2     (*((psint)(PBSTART+0x164)))  /*                       Переключатель ВРЕМЯ СРАБАТЫВАНИЯ от ВУ ИС */
#define signal_A2IS12LDU     (*((psbool)(PBSTART+0x167)))  /*                       Магнит РБ1 зацеплен */
#define signal_B2IS12LDU     (*((psbool)(PBSTART+0x169)))  /*                       Магнит РБ2 зацеплен */
#define signal_R0EE02LDU     (*((psbool)(PBSTART+0x16b)))  /*                       Питание  АКНП  отключить */
...
void __stdcall CheckSignals(DVE_Parametrs *Par)
{
  ...
  CheckSInt(0);
#else
ParExchange("A0IT02IZ2",2,&SignalBuffer[0],"Температура АЗ1-2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(16);
#else
...
ParExchange("A1VP81LZZ",3,&SignalBuffer[18],"Давление СБРОС ББ1 в норме",0);
  ...
  CheckSInt(42);
#else
ParExchange("A2IP01IZ2",2,&SignalBuffer[42],"Текущее давление СБРОС РБ1",0);
#endif
#ifndef SIMUL_MODE
  CheckSInt(45);
#else
ParExchange("B2IP01IZ2",2,&SignalBuffer[45],"Текущее давление СБРОС РБ2",0);
#endif
#ifndef SIMUL_MODE
  CheckSInt(50);
#else
ParExchange("B0IT02IZ2",2,&SignalBuffer[50],"Температура АЗ2-2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(57);
#else
...
ParExchange("A3IS22LDU",3,&SignalBuffer[192],"Приход на НУП ИС1",0);
  ...
  CheckSBool(194);
#else
ParExchange("R0AD03LZ2",3,&SignalBuffer[194],"Имитация прихода на ВУ ИС",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(196);
#else
ParExchange("B3IS11LDU",3,&SignalBuffer[196],"Приход на ВУ ИС2",0);
  ...
  CheckSBool(202);
#else
ParExchange("R0AD05LZ2",3,&SignalBuffer[202],"Имитация ухода с НУП ИС",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(208);
#else
ParExchange("B3IS22LDU",3,&SignalBuffer[208],"Приход на НУП ИС2",0);
  ...
  CheckSBool(225);
#else
ParExchange("A1EE01LS1",3,&SignalBuffer[225],"Исправность АКНП канал 1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(227);
#else
ParExchange("A1EE01LS4",3,&SignalBuffer[227],"Исправность АКНП канал 4",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(229);
#else
ParExchange("A1EE01LS3",3,&SignalBuffer[229],"Исправность АКНП канал 3",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(239);
#else
ParExchange("A1EE01LS2",3,&SignalBuffer[239],"Исправность АКНП  канал 2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(241);
#else
ParExchange("A0EE05LZ2",3,&SignalBuffer[241],"Исправность сети АКНП1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(243);
#else
ParExchange("A0EE06LZ2",3,&SignalBuffer[243],"Исправность сети АКНП2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(245);
#else
ParExchange("A0EE07LZ2",3,&SignalBuffer[245],"Исправность сети АКНП3",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(247);
#else
ParExchange("A0EE08LZ2",3,&SignalBuffer[247],"Исправность сети АКНП4",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(286);
#else
ParExchange("A1VN71LS1",3,&SignalBuffer[286],"Блокировка автоматического подъёма ББ канал 1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(288);
#else
ParExchange("A1VN71LS2",3,&SignalBuffer[288],"Блокировка автоматического подъёма ББ канал 2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(290);
#else
ParExchange("A1VN71LS3",3,&SignalBuffer[290],"Блокировка автоматического подъёма ББ канал 3",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(292);
#else
ParExchange("A1VN71LS4",3,&SignalBuffer[292],"Блокировка автоматического подъёма ББ канал 4",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(300);
#else
ParExchange("R0VW13LDU",3,&SignalBuffer[300],"Индикация выбора АЗ1 или АЗ1+АЗ2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(302);
#else
ParExchange("R0VW23LDU",3,&SignalBuffer[302],"Индикация выбора АЗ2 или АЗ1+АЗ2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(304);
#else
ParExchange("R7II73LZ2",3,&SignalBuffer[304],"Сработала АС II УР РАД",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(306);
#else
ParExchange("R7II71LZ2",3,&SignalBuffer[306],"Сработала АС IУР РАД (датчик 1)",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(308);
#else
ParExchange("R7II72LZ2",3,&SignalBuffer[308],"Сработала АС IУР РАД (датчик 2)",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(310);
#else
ParExchange("R0AD21LDU",3,&SignalBuffer[310],"Подключить защиту от II УР",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(312);
#else
ParExchange("R0MD34LP1",3,&SignalBuffer[312],"Кнопка Квитировать",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(314);
#else
ParExchange("R0AD14LDU",3,&SignalBuffer[314],"Имитация срабатывания верхней АС II УР",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(316);
#else
ParExchange("R0AD16LDU",3,&SignalBuffer[316],"Имитация срабатывания верхней АС I УР",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(318);
#else
ParExchange("R6IS66LZZ",3,&SignalBuffer[318],"Исправность ВИП 4,0 (№5) ССДИ-1-1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(320);
#else
ParExchange("R6IS67LZZ",3,&SignalBuffer[320],"Исправность ВИП 4,0 (№6) ССДИ-1-2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(322);
#else
ParExchange("R6IS68LZZ",3,&SignalBuffer[322],"Исправность ВИП 4,0 (№7) ССДИ-2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(324);
#else
ParExchange("R0VN71LZ2",3,&SignalBuffer[324],"АС по мощности канал 1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(326);
#else
ParExchange("R0VN75LZ2",3,&SignalBuffer[326],"АС по периоду разгона канал 1",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(328);
#else
ParExchange("R0VN72LZ2",3,&SignalBuffer[328],"АС по мощности канал 2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(330);
#else
ParExchange("R0VN76LZ2",3,&SignalBuffer[330],"АС по периоду разгона канал  2",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(332);
#else
ParExchange("R0VN73LZ2",3,&SignalBuffer[332],"АС по мощности канал 3",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(334);
#else
ParExchange("R0VN74LZ2",3,&SignalBuffer[334],"АС по мощности канал 4",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(336);
#else
ParExchange("R0VN77LZ2",3,&SignalBuffer[336],"АС по периоду разгона канал  3",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(338);
#else
ParExchange("R0VN78LZ2",3,&SignalBuffer[338],"АС по периоду разгона канал  4",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(340);
#else
ParExchange("R0AD04LZ2",3,&SignalBuffer[340],"Имитация срабатывания верхней АС II УР",0);
#endif
#ifndef SIMUL_MODE
  CheckSInt(356);
#else
ParExchange("R0MW12IP2",2,&SignalBuffer[356],"Переключатель ВРЕМЯ СРАБАТЫВАНИЯ от ВУ ИС",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(359);
#else
ParExchange("A2IS12LDU",3,&SignalBuffer[359],"Магнит РБ1 зацеплен",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(361);
#else
ParExchange("B2IS12LDU",3,&SignalBuffer[361],"Магнит РБ2 зацеплен",0);
#endif
#ifndef SIMUL_MODE
  CheckSBool(363);
#else
ParExchange("R0EE02LDU",3,&SignalBuffer[363],"Питание  АКНП  отключить",0);
#endif
#ifndef SIMUL_MODE
...
void UnloadSignals()
{
ParExchange("A0CT01IZ2",1,&SignalBuffer[3],"Температура АЗ1-1",1);
ParExchange("A0VT61LZ2",3,&SignalBuffer[8],"ПС по температуре в АЗ1",1);
ParExchange("A0VT71LZ2",3,&SignalBuffer[10],"АС по температуре в АЗ1",1);
ParExchange("A1VP41LZ2",3,&SignalBuffer[12],"Давление СБРОС ББ1 ниже АС",1);
ParExchange("B1VP41LZ2",3,&SignalBuffer[14],"Давление СБРОС ББ2 ниже АС",1);
ParExchange("A2VP41LZ2",3,&SignalBuffer[20],"Давление СБРОС РБ1 ниже АС",1);
ParExchange("A2CP01RZ2",1,&SignalBuffer[22],"Текущее давление СБРОС РБ1",1);
ParExchange("A2VP51LZ2",3,&SignalBuffer[27],"Давление СБРОС РБ1 ниже ПС",1);
ParExchange("B2VP41LZ2",3,&SignalBuffer[29],"Давление СБРОС РБ2 ниже АС",1);
ParExchange("B2CP01RZ2",1,&SignalBuffer[31],"Текущее давление СБРОС РБ2",1);
ParExchange("B2VP51LZ2",3,&SignalBuffer[36],"Давление СБРОС РБ2 ниже ПС",1);
ParExchange("B2VP81LZ2",3,&SignalBuffer[38],"Давление СБРОС РБ2 в норме",1);
ParExchange("A2VP81LZ2",3,&SignalBuffer[40],"Давление СБРОС РБ1 в норме",1);
ParExchange("R0ET01LZ2",3,&SignalBuffer[48],"Признак наличия неисправности по температуре  АЗ1",1);
ParExchange("A0VP41LZ2",3,&SignalBuffer[53],"Давление АЗ1 ниже АС",1);
ParExchange("B0VP41LZ2",3,&SignalBuffer[55],"Давление АЗ2 ниже АС",1);
ParExchange("R0ET02LZ2",3,&SignalBuffer[61],"Признак наличия неисправности по температуре АЗ2",1);
ParExchange("B0EP02IZ2",2,&SignalBuffer[63],"Индикатор -Давление  АЗ2 ниже АС",1);
ParExchange("A0EP02IZ2",2,&SignalBuffer[66],"Индикатор -Давление  АЗ1 ниже АС",1);
ParExchange("A0VP71LZ2",3,&SignalBuffer[69],"АЗ по АС любого давления для АЗ1",1);
ParExchange("B0VP71LZ2",3,&SignalBuffer[71],"АЗ по АС любого давления для АЗ2",1);
ParExchange("R0VN01RZ2",1,&SignalBuffer[129],"Уровень мощности по каналу 1",1);
ParExchange("R0VN02RZ2",1,&SignalBuffer[134],"Уровень мощности по каналу 2",1);
ParExchange("R0VN03RZ2",1,&SignalBuffer[139],"Уровень мощности по каналу 3",1);
ParExchange("R0VN04RZ2",1,&SignalBuffer[144],"Уровень мощности по каналу 4",1);
ParExchange("R0VN05RZ2",1,&SignalBuffer[149],"Период разгона по каналу 1",1);
ParExchange("R0VN06RZ2",1,&SignalBuffer[154],"Период разгона по каналу 2",1);
ParExchange("R0VN07RZ2",1,&SignalBuffer[159],"Период разгона по каналу 3",1);
ParExchange("R0VN08RZ2",1,&SignalBuffer[164],"Период разгона по каналу 4",1);
ParExchange("R0VN61LZ2",3,&SignalBuffer[169],"ПС по мощности канал 1",1);
ParExchange("R0VN62LZ2",3,&SignalBuffer[171],"ПС по мощности канал 2",1);
ParExchange("R0VN63LZ2",3,&SignalBuffer[173],"ПС по мощности канал 3",1);
ParExchange("R0VN64LZ2",3,&SignalBuffer[175],"ПС по мощности канал 4",1);
ParExchange("R0VN65LZ2",3,&SignalBuffer[177],"ПС по периоду разгона канал 1",1);
ParExchange("R0VN66LZ2",3,&SignalBuffer[179],"ПС по периоду разгона канал 2",1);
ParExchange("R0VN67LZ2",3,&SignalBuffer[181],"ПС по периоду разгона канал 3",1);
ParExchange("R0VN68LZ2",3,&SignalBuffer[183],"ПС по периоду разгона канал 4",1);
ParExchange("R0VN09RZ2",1,&SignalBuffer[185],"Усредненный период разгона",1);
ParExchange("A3VZ31LZ2",3,&SignalBuffer[198],"АЗ от ВУ ИС1",1);
ParExchange("B3VZ31LZ2",3,&SignalBuffer[200],"АЗ от ВУ ИС2",1);
ParExchange("A3VZ11LZ2",3,&SignalBuffer[204],"АЗ от НУП ИС1",1);
ParExchange("B3VZ11LZ2",3,&SignalBuffer[206],"АЗ от НУП ИС2",1);
ParExchange("R0VN69LZ2",3,&SignalBuffer[210],"ПС по усредненному периоду",1);
ParExchange("R0VN79LZ2",3,&SignalBuffer[212],"АС по усредненному периоду",1);
ParExchange("R0VR02RZ2",1,&SignalBuffer[214],"Усредненная реактивность",1);
ParExchange("R0VZ71LZ2",3,&SignalBuffer[219],"Обобщенный сигнал АЗ 2 канала",1);
ParExchange("R0VN70LZ2",3,&SignalBuffer[221],"АЗ по АС мощности РУ",1);
ParExchange("R0VN80LZ2",3,&SignalBuffer[223],"АЗ по АС периода разгона РУ",1);
ParExchange("A0EE01LZ2",3,&SignalBuffer[231],"Исправность АКНП1",1);
ParExchange("A0EE04LZ2",3,&SignalBuffer[233],"Исправность АКНП4",1);
ParExchange("A0EE02LZ2",3,&SignalBuffer[235],"Исправность АКНП2",1);
ParExchange("A0EE03LZ2",3,&SignalBuffer[237],"Исправность АКНП3",1);
ParExchange("A0VZ71LZ2",3,&SignalBuffer[249],"Обобщенный сигнал АС по АЗ1",1);
ParExchange("A0EE00LZ2",3,&SignalBuffer[251],"Исправность АКНП АЗ1",1);
ParExchange("B0EE00LZ2",3,&SignalBuffer[253],"Исправность АКНП АЗ2",1);
ParExchange("B0VZ71LZ2",3,&SignalBuffer[255],"Обобщенный сигнал АС по АЗ2",1);
ParExchange("A0VN71LZ2",3,&SignalBuffer[257],"АЗ по АС мощности  АЗ1",1);
ParExchange("B0VN71LZ2",3,&SignalBuffer[259],"АЗ по АС мощности  АЗ2",1);
ParExchange("A0VN81LZ2",3,&SignalBuffer[261],"АЗ по АС периода разгона АЗ1",1);
ParExchange("B0VN81LZ2",3,&SignalBuffer[263],"АЗ по АС периода разгона АЗ2",1);
ParExchange("R0VZ05UZ2",7,&SignalBuffer[265],"Индикатор причины сброса",1);
ParExchange("R0VN11RZ2",1,&SignalBuffer[270],"Средняя мощность по БАЗ1",1);
ParExchange("R0VP73LZ2",3,&SignalBuffer[275],"ПС давления для РУ",1);
ParExchange("B0CT01IZ2",1,&SignalBuffer[277],"Температура АЗ2-1",1);
ParExchange("B0VT61LZ2",3,&SignalBuffer[282],"ПС по температуре в АЗ2",1);
ParExchange("B0VT71LZ2",3,&SignalBuffer[284],"АС по температуре в АЗ2",1);
ParExchange("A1VN71LZ2",3,&SignalBuffer[294],"Блокировка автоматического подъёма ББ",1);
ParExchange("A3VZ15LZ2",3,&SignalBuffer[296],"Имитация АЗ от НУП ИС",1);
ParExchange("A3VZ13LZ2",3,&SignalBuffer[298],"Имитация АЗ от ВУ ИС",1);
ParExchange("R0EE01LZ2",3,&SignalBuffer[342],"Питание  АКНП1  отключить",1);
ParExchange("R0EE02LZ2",3,&SignalBuffer[344],"Питание  АКНП2  отключить",1);
ParExchange("R0EE03LZ2",3,&SignalBuffer[346],"Питание  АКНП3  отключить",1);
ParExchange("R0EE04LZ2",3,&SignalBuffer[348],"Питание  АКНП4  отключить",1);
ParExchange("R0AD14LZ2",3,&SignalBuffer[350],"Имитация АЗ от IIУР",1);
ParExchange("R0AD15LZ2",3,&SignalBuffer[352],"Имитация АЗ от 1УР (датчик1)",1);
ParExchange("R0AD16LZ2",3,&SignalBuffer[354],"Имитация АЗ от 1УР (датчик2)",1);
}
#endif
...
NewEEPROM("fEM_A1UL11RZZ",1,&fEM_A1UL11RZZ,"Время задержки АЗ от ВУ ИС","0.020");
