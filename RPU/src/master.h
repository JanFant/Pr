...
static ModbusRegister hr_RPU[]={
	...
	{&A0IT03IRP,3,1},	//( - K01VASR, RPU) Температура АЗ1-3
	{&B8VC01RDU,8,2},	//( - , RPU) Координата АЗ2, мм
	{&fEM_R0UH02RSS,8,4},	//( - , RPU) Коэфф. преобразования частота->нейтр/с КНК15-1
	{&fEM_R0UH03RSS,8,6},	//( - , RPU) Коэфф. преобразования частота->нейтр/с КНК53М
	{&fEM_R7UX00RSS,8,8},	//( - , RPU) X-координата АЗ1 (см)
	{&fEM_R7UY00RSS,8,10},	//( - , RPU) Y-координата АЗ1 (см)
	{&fEM_R7UX13RSS,8,12},	//( - , RPU) X-координата камеры R7IN51
	{&fEM_R7UX14RSS,8,14},	//( - , RPU) X-координата камеры R7IN52
	{&fEM_R7UX15RSS,8,16},	//( - , RPU) X-координата камеры R7IN53
	{&fEM_R7UY13RSS,8,18},	//( - , RPU) Y-координата камеры R7IN51
	{&fEM_R7UY14RSS,8,20},	//( - , RPU) Y-координата камеры R7IN52
	{&fEM_R7UY15RSS,8,22},	//( - , RPU) Y-координата камеры R7IN53
	{&fEM_A0UX00RSS,8,24},	//( - , RPU) Эффективный радиус АЗ
	{&fEM_A0UX13RSS,8,26},	//( - , RPU) Первый коэффициент калибровки камеры 13
	{&fEM_A0UX14RSS,8,28},	//( - , RPU) Первый коэффициент калибровки камеры 14
	{&fEM_A0UX15RSS,8,30},	//( - , RPU) Первый коэффициент калибровки камеры 15
	{&fEM_B0UX03RSS,8,32},	//( - , RPU) Второй коэффициент калибровки камеры 3
	{&fEM_B0UX04RSS,8,34},	//( - , RPU) Второй коэффициент калибровки камеры4
	{&fEM_B0UX05RSS,8,36},	//( - , RPU) Второй коэффициент калибровки камеры 5
	{&fEM_R0UH05RSS,8,38},	//( - , RPU) Кол-во делений/сек на 1 ватт * E^6
	{&fEM_R0UR01RRP,8,40},	//( - , RPU) Уставка СНМ11 в БЗ - АС по частоте
	{&fEM_R0UT02RZZ,8,42},	//( - , RPU) Верхний предел шкалы датчика температуры
	{&fEM_R0UT01RZZ,8,44},	//( - , RPU) Нижний предел шкалы датчика температуры
	{&fEM_R0UT72RZZ,8,46},	//( - , RPU) Граница неоднозначности срабатывания аварийной сигнализации по температуре АЗ (град)
	{&fEM_R0UT71RZZ,8,48},	//( - , RPU) Уровень срабатывания аварийной сигнализации по температуре АЗ
	{&fEM_R0UT62RZZ,8,50},	//( - , RPU) Граница неоднозначности срабатывания предупредительной сигнализации по температуре АЗ (град)
	{&fEM_R0UT61RZZ,8,52},	//( - , RPU) Уровень срабатывания предупредительной сигнализации по температуре АЗ
	{&fEM_R0UL52RSS,8,54},	//( - , RPU) Уровень АС по мощности
	{&fEM_Z7UE20RRP,8,56},	//( - , RPU) Время задержки сигнала на включение источников питания после снятия команды на отключение
	{&fEM_R0UH21RSS,8,58},	//( - , RPU) Верхняя граница измерения частоты импульсов(имп/с) СНМ-11
	{&fEM_R0UH22RSS,8,60},	//( - , RPU) Верхняя граница измерения частоты импульсов(имп/с) КНК-15-1
	{&fEM_R0UH23RSS,8,62},	//( - , RPU) Верхняя граница измерения частоты импульсов(имп/с) КНК-53М
	{&fEM_R0IN11NRP,8,64},	//( - , RPU) Коэффициент A (РПУ к 1)
	{&fEM_R0IN12NRP,8,66},	//( - , RPU) Коэффициент B (РПУ к 1)
	{&fEM_R0IN21NRP,8,68},	//( - , RPU) Коэффициент A (РПУ к 2)
	{&fEM_R0IN22NRP,8,70},	//( - , RPU) Коэффициент B (РПУ к 2)
	{&fEM_R0IN31NRP,8,72},	//( - , RPU) Коэффициент A (РПУ к 3)
	{&fEM_R0IN32NRP,8,74},	//( - , RPU) Коэффициент B (РПУ к 3)
	{&fEM_R0IN61NRP,8,76},	//( - , RPU) Коэффициент A (РПУ к 6)
	{&fEM_R0IN62NRP,8,78},	//( - , RPU) Коэффициент B (РПУ к 6)
	{&fEM_R0IN71NRP,8,80},	//( - , RPU) Коэффициент A (РПУ к 7)
	{&fEM_R0IN72NRP,8,82},	//( - , RPU) Коэффициент B (РПУ к 7)
	{NULL,0,0},
};
#pragma pop
#pragma pack(push,1)
